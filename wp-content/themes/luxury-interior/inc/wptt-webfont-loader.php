...
if ( ! class_exists( 'Luxury_Interior_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Luxury_Interior_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $luxury_interior_url The remote URL.
		 */
		public function __construct( $luxury_interior_url = '' ) {
			$this->remote_url = $luxury_interior_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $luxury_interior_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $luxury_interior_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $luxury_interior_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $luxury_interior_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $luxury_interior_url ] );
					}
				}
		 ...
		 * @access protected
		 *
		 * @param string $luxury_interior_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $luxury_interior_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $luxury_interior_url, '/' ) ) {
				$parsed_url = parse_url( $this->remote_url );
				return $parsed_url['scheme'] . '://' . $parsed_url['hostname'] . $luxury_interior_url;
			}

			return $luxury_interior_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $luxury_interior_url    The URL of the remote webfont.
	 * @param string $format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $luxury_interior_url, $format = 'woff2' ) {
		$font = new Luxury_Interior_WPTT_WebFont_Loader( $luxury_interior_url );
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $luxury_interior_url, $format = 'woff2' ) {
		$font = new Luxury_Interior_WPTT_WebFont_Loader( $luxury_interior_url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
